(() => {
    ...
    core.use([ Mousewheel, Navigation ]);
    document.addEventListener("DOMContentLoaded", (() => {
        const swiperMini = new core(".swiper-mini", {
            loop: false,
            slidesPerView: 3,
            spaceBetween: 20,
            centeredSlides: true,
        ...
        let item = document.querySelectorAll(".person-item-wrap");
        let images = document.querySelectorAll(".swiper-mini-slide img");
        let imgChild = document.querySelectorAll(".person-item-wrap .slide-img");
        swiperMini.on("slideChange", (function() {
            let i = swiperMini.realIndex;
            if (item[i].previousElementSibling) item[i].previousElementSibling.style.display = "none";
            item[i].style.display = "block";
            if (item[i].nextElementSibling) item[i].nextElementSibling.style.display = "none";
            let image = images[i].src;
            imgChild[i].src = image;
        }));
    }));
